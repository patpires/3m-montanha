document.addEventListener('DOMContentLoaded', () => { const jogo = document.getElementById('jogo');
 ...
 let vidas = 1;
 let tempoTotal = 120;
 // Tempo total em segundos (3 minutos = 180 segundos) let tempoRestante = tempoTotal;
 // Inicializa o tempo restante com o tempo total
...
const painelVidas = document.createElement('div');
 ...
 painelVidas.style.background = 'rgba(245, 245,245, 0.7)' painelVidas.style.color = 'black';
...
// Previne o comportamento padrão de toque em toda a tela, exceto em botões document.addEventListener('touchstart', function(e) { if (!e.target.matches('button')) { e.preventDefault();
 } }, { passive: false });


// Supondo que 'jogo' seja um elemento já existente no seu HTML onde você deseja adicionar os pontos pontosDeVida.forEach(ponto => { const elementoPonto = document.createElement('div');
 // Adiciona a classe .ponto ao elemento elementoPonto.classList.add('ponto');
 // Define as posições x e y usando estilos CSS elementoPonto.style.left = ${ponto.x}px;
...
function updateJoaninhaPosition() { joaninhaPos.x = Math.max(0, Math.min(jogo.offsetWidth - joaninha.offsetWidth, joaninhaPos.x));
...
const step = 10;
 // Isso define o quanto a joaninha se move

function moveLeft() { if (!playing) return;
 joaninhaPos.x -= step;
 updateJoaninhaPosition();
 }

function moveRight() { if (!playing) return;
 joaninhaPos.x += step;
 updateJoaninhaPosition();
 }

function moveUp() { if (!playing) return;
 joaninhaPos.y -= step;
 updateJoaninhaPosition();
 }

function moveDown() { if (!playing) return;
 joaninhaPos.y += step;
 updateJoaninhaPosition();
 }

document.addEventListener('keydown', (e) => { switch(e.key) { case 'ArrowUp': moveUp();
 break;
...
function createObstacle() { const obstacleIndex = Math.floor(Math.random() * obstacleGifs.length);
...
function createRestartButton(text) { document.querySelectorAll('.obstacle').forEach(obstacle => obstacle.remove());
...
function victory() { playing = false;
 createRestartButton('Você venceu! Conseguiu ficar por 2 minutos! Clique para jogar novamente');
 }

// Reconstrução da função checkCollision function checkCollision() { const joaninhaRect = joaninha.getBoundingClientRect();
 // Certifique-se de que joaninha é um elemento DOM válido
...
function atualizarPainel() { let minutos = Math.floor(tempoRestante / 60);
...
// Reconstrução da função gameLoop function gameLoop() { if (!playing) return;
 checkCollision();
 requestAnimationFrame(gameLoop);
 // Adiciona continuidade ao loop do jogo }

//codigo novo document.getElementById('left-btn').addEventListener('click', moveLeft);
...
const timer = setInterval(() => { if (tempoRestante > 0) { tempoRestante--;
 // Decrementa o tempo restante atualizarPainel();
 // Atualiza o painel com o tempo restante } else { clearInterval(timer);
 // Para o timer quando o tempo acabar // Aqui você pode adicionar qualquer lógica adicional que deve acontecer quando o tempo acabar //alert("O tempo acabou! Tente novamente.");
 } }, 1000);
